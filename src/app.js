...
function getData(data) {
    ...
    data.map((data) => {
        ...
        div.innerHTML = `<div class="first col-md-6 container-fluid mb-md-3 mb-2 bg-white shadow-lg   ">
        <img src=${data.logo} alt=${data.position} class="my-img d-block">
        <h3  class=" d-inline company ">${data.company}</h3>
        <h3  class="d-minline badge badge-pill new "id=${data.new?"new":"none"} >${data.new ?"NEW!":''}</h3>
        <h3  class=" d-inline badge badge-pill   featured " id=${data.featured?"featured":"none"}>${data.featured?"FEATURED":''}</h3>
        <h3  class="d-block" style="font-weight:bolder; color:black; padding-top:15px;"> ${data.position}</h3>
        <h3 class="d-inline">${data.postedAt}</h3>
        <h3  class="d-inline">${data.contract}</h3>
        <h3 class="d-inline">${data.location}</h3>
        </div>
        <div class="second col-md-6  container-fluid mb-md-3  mb-2 bg-white shadow-lg d-inline-block ">
